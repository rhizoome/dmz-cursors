...
class SVGRect:
    ...
    def renderFromSVG(self, svgFName, sliceFName):
        for size in (24, 28, 32, 40, 48, 56, 64, 72, 80, 88, 96):
            os.system("mkdir -p pngs/%sx%s" % (size, size))
            rc = os.system('inkscape --without-gui -w %s -h %s --export-id="%s" --export-png="pngs/%sx%s/%s" "%s"' % (size, size, self.name, size, size, sliceFName, svgFName))
            if rc > 0:
